...
# Lazy-load torch to avoid CUDA initialization on import
# This is necessary because torch.cuda.is_available() can hang
# We'll import torch only when needed
torch = None
nn = None
optim = None
amp = None

from models.autoencoders import VoteAutoencoder
...
from data_collectors.external_signals import (
...
# Configuration
DEVICE = "cpu"  # Default to CPU to avoid CUDA initialization issues
TORCH_DTYPE = None
USE_AMP = False
...
def _ensure_torch():
    """Lazy-load torch only when needed and configure device/dtype."""
    global torch, nn, optim, amp, DEVICE, TORCH_DTYPE, USE_AMP
    if torch is None:
        try:
            ...
            try:
                ...
                logger.debug(f"Device resolution failed: {e}. Using CPU.")
                DEVICE = torch_module.device("cpu")

            TORCH_DTYPE = torch_module.float16 if DEVICE.type == "cuda" else torch_module.float32
            USE_AMP = DEVICE.type == "cuda"
        except ImportError:
            logger.warning("PyTorch not installed. Running without GPU support.")
            torch = None
            nn = None
            optim = None
            amp = None
...
class CongressMenBuilder:
    ...
    def _save_cache(self) -> None:
        ...
        try:
            ...
            if hasattr(self, 'ideal_point_model') and self.ideal_point_model is not None:
                _ensure_torch()
                torch.save(
                    self.ideal_point_model.state_dict(),
                    os.path.join(cache_dir, f"{self.CACHE_PREFIX}_model.pt")
                )
    ...
    def _load_cache(self) -> None:
        ...
        try:
            ...
            model_path = os.path.join(cache_dir, f"{self.CACHE_PREFIX}_model.pt")
            if os.path.exists(model_path):
                _ensure_torch()
                if torch is not None:
                    model = IdealPointModel(len(self.leg_ids), len(self.vote_ids), dim=3)
    ...
    def _fetch(self):
        ...
        try:
            ...
            logger.info(f"[OK] Loaded {len(df)} voting records from Congress #{CONGRESS_NUMBER} (pyvoteview)")
    ...
    def _train_autoencoder(self, latent_dim=8, epochs=40):
        _ensure_torch()
        if torch is None:
            logger.warning("PyTorch not available. Returning random embeddings instead of trained autoencoder.")
            return np.random.randn(len(self.leg_ids), latent_dim).astype(np.float32)

        X = StandardScaler().fit_transform(self.vote_matrix)
        X_tensor = torch.tensor(X, dtype=torch.float32, device=DEVICE)

        batch_size = min(512, max(64, X_tensor.shape[0]))
        dataset = torch.utils.data.TensorDataset(X_tensor)
        loader = torch.utils.data.DataLoader(dataset, batch_size=batch_size, shuffle=True, drop_last=False)

        model = VoteAutoencoder(X_tensor.shape[1], latent_dim).to(DEVICE)
    ...
    def _train_ideal_points(self, dim=3, epochs=30, lr=1e-2, batch_size: int = 2048):
        """Train Ideal Point Model with convergence logging.
        ...
        Returns:
            Trained ideal points as numpy array of shape (n_legislators, dim)
        """
        _ensure_torch()
        if torch is None:
            logger.warning("PyTorch not available. Returning random ideal points instead of trained model.")
            self.ideal_point_model = None
            return np.random.randn(len(self.leg_ids), dim).astype(np.float32)
        ...
        li = torch.tensor(df["li"].values, device=DEVICE, dtype=torch.long)
        vi = torch.tensor(df["vi"].values, device=DEVICE, dtype=torch.long)
        y = torch.tensor(df["v"].values, device=DEVICE, dtype=torch.float32)

        dataset = torch.utils.data.TensorDataset(li, vi, y)
        eff_batch_size = max(256, min(batch_size, len(df)))
        loader = torch.utils.data.DataLoader(dataset, batch_size=eff_batch_size, shuffle=True, drop_last=False)

        model = IdealPointModel(len(self.leg_ids), len(self.vote_ids), dim).to(DEVICE)
        opt = optim.Adam(model.parameters(), lr=lr)
        loss_fn = nn.BCELoss()
        scaler = amp.GradScaler(enabled=USE_AMP)

        logger.info(f"Training IPM: {len(self.leg_ids)} legislators, {len(self.vote_ids)} votes, dim={dim}")
        ...
        for e in range(epochs):
            epoch_losses = []
            for batch_li, batch_vi, batch_y in loader:
                opt.zero_grad(set_to_none=True)
                with amp.autocast(enabled=USE_AMP):
                    p = model(batch_li, batch_vi)
            ...
            loss_value = float(torch.stack(epoch_losses).mean().item()) if epoch_losses else 0.0
            logger.info(f"Epoch {e+1:3d}/{epochs}: Loss = {loss_value:.6f}")
            
            # Check for convergence
            if abs(prev_loss - loss_value) < 1e-5:
                patience_counter += 1
                if patience_counter >= patience:
                    logger.info(f"[OK] Loss converged at epoch {e+1}. Stopping early.")
                    break
            else:
                patience_counter = 0
        ...
        self.ideal_point_model = model.cpu()
        
        logger.info(f"[OK] IPM training complete. Final loss: {prev_loss:.6f}")
    ...
    def export_actors_with_model(self):
        ...
        if hasattr(self, 'ideal_point_model') and self.ideal_point_model is not None:
            try:
                # Extract salience (a_j) and threshold (b_j) from IPM
                salience = self.ideal_point_model.a.detach().cpu().numpy()  # Shape: (n_votes, dim)
                threshold = self.ideal_point_model.b.detach().cpu().numpy()  # Shape: (n_votes,)
                
                voting_params = {
                    'salience': salience,  # a_j: discriminant/salience of each vote
                    'threshold': threshold  # b_j: difficulty parameter of each vote
                }
