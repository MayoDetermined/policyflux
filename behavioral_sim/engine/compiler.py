...
from behavioral_sim.network import DynamicNetwork, InfluenceFunction
...
class CongressCompiler:
    ...
    def compile(
        ...
        if simulator.congress is not None:
            base_adj = torch.as_tensor(simulator.congress.base_adj_matrix, dtype=torch.float32, device=self.device)
