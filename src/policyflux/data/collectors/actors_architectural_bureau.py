...
from policyflux.data import ExternalSignalCollector
...
from policyflux.defaults import SIMULATION, MODELS, PATHS  # from policyflux import configuration

logger = logging.getLogger(__name__)

# Configuration

CONGRESS_NUMBER = defaults.CONGRESS_NUMBER  # Use config for Congress number
NORMALIZE_BEHAVIORAL_FEATURES = defaults.NORMALIZE_BEHAVIORAL_FEATURES
NORMALIZATION_TYPE = defaults.NORMALIZATION_TYPE
AUTOENCODER_LATENT_DIM = defaults.AUTOENCODER_LATENT_DIM


class CongressMenBuilder:
    CACHE_DIR = "cache"
    CACHE_PREFIX = f"congress_{defaults.CONGRESS_NUMBER}"
    ...
    def __init__(
        ...
        if use_cache and self._cache_exists():
            logger.info(f"Loading cached data for Congress #{defaults.CONGRESS_NUMBER}...")
            self._load_cache()
    ...
    def _fetch(self):
        ...
        try:
            ...
            logger.info(f"[OK] Loaded {len(df)} voting records from policyflux.environment #{CONGRESS_NUMBER} (pyvoteview)")
    ...
    def export_actors_with_model(self):
        ...
        signals_bundle = self.external_signals.collect(self.leg_ids)
