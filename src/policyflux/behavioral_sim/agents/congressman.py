...
from policyflux.behavioral_sim.agents.base import AbstractAgent, BehavioralParameters
from policyflux.congress.actors import CongressMan as LegacyCongressMan
...
class CongressAgent(LegacyCongressMan, AbstractAgent):
    ...
    def compute_utility(self, law: Any, context: Any, network_view: Any) -> float:
        ...
        if network_view is not None:
            try:
                if isinstance(network_view, tf.Tensor):
                    network_term = float(tf.reduce_mean(network_view).numpy())
                else:
    ...
    def to_tensor(self, device: str) -> tf.Tensor:
        with tf.device(device):
            arr = tf.convert_to_tensor(self.ideology, dtype=tf.float32)
            return tf.reshape(arr, (1, -1))
