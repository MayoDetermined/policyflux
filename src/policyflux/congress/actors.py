...
from policyflux.models.rnn import ActorLSTM, ActorLSTMTrainer
from policyflux.models.dqn import VoteDQN, DQNAgent
...
class CongressMan:
    ...
    def __init__(self, id: int, data: Dict, ideal_point_model: 'IdealPointModel' = None) -> None:
        ...
        self.last_state: Optional[tf.Tensor] = None  # For storing state for learning
    ...
    def _get_rnn_input_data(self, current_pressure: float) -> tf.Tensor:
        ...
        for ideology_vec in self.ideology_history:
            ...
            if isinstance(ideology_vec, tf.Tensor):
        ...
        history_tensor = tf.convert_to_tensor(history_array, dtype=tf.float32)
        history_tensor = tf.expand_dims(history_tensor, axis=0)
    ...
    def train_rnn_step(self, target_ideology: np.ndarray, current_pressure: float) -> Optional[float]:
        ...
        try:
            ...
            # Prepare target
            target_tensor = tf.convert_to_tensor(target_ideology, dtype=tf.float32)
    ...
    def predict_ideology_rnn(self, current_pressure: float) -> np.ndarray:
        ...
        try:
            ...
            return predicted.numpy().squeeze(0)
        except Exception as e:
            import logging
            logger = logging.getLogger(__name__)
    ...
    def record_dqn_reward(
        self,
        next_state: Optional[tf.Tensor] = None,
        done: bool = False
    ) -> None:
        ...
        if next_state is None:
            next_state = tf.zeros_like(self.last_state)
        
        try:
            # Add to replay buffer
            self.dqn_agent.remember(
                state=self.last_state.numpy(),
